// components/SzeneKarte.tsx
'use client';

import Image from 'next/image';
import Link from 'next/link';
import { useEffect, useState } from 'react';

interface SzeneKarteProps {
  szene: {
    id: number;
    titel: string;
    beschreibung: string;
    thumbnail_path?: string;
    themen?: string;
  };
}

export default function SzeneKarte({ szene }: SzeneKarteProps) {
  const [signedUrl, setSignedUrl] = useState<string | null>(null);

  const begrenzteBeschreibung = szene.beschreibung?.slice(0, 50) +
    (szene.beschreibung?.length > 50 ? '...' : '');

  useEffect(() => {
    const fetchSignedUrl = async () => {
      if (!szene.thumbnail_path) return;

      try {
        const res = await fetch('/api/signed-url', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ key: szene.thumbnail_path })
        });

        const json = await res.json();
        if (json.url) setSignedUrl(json.url);
      } catch (err) {
        console.warn('Fehler beim Laden der Thumbnail-URL:', err);
      }
    };

    fetchSignedUrl();
  }, [szene.thumbnail_path]);

  const parsedThemen = Array.isArray(szene.themen)
  ? szene.themen
  : (() => {
      try {
        const parsed = JSON.parse(szene.themen || '[]');
        return Array.isArray(parsed) ? parsed : [];
      } catch {
        return [];
      }
    })();


  return (
    <Link
      href={`/videoszene/${szene.id}`}
      className="block w-full sm:w-1/2 md:w-1/4 p-2"
    >
      <div className="flex flex-col h-full bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
        {signedUrl && (
          <div className="relative h-48 w-full">
            <Image
              src={signedUrl}
              alt={szene.titel}
              fill
              className="object-cover"
            />
          </div>
        )}
        <div className="p-4 flex flex-col flex-grow justify-between">
          <div>
            <h3 className="text-lg font-semibold mb-1 line-clamp-2">{szene.titel}</h3>
            <p className="text-sm text-gray-600 mb-3 line-clamp-2">{begrenzteBeschreibung}</p>
          </div>
          <div className="flex flex-wrap gap-1 mt-auto">
            {parsedThemen.map((tag: string, idx: number) => (
              <span
                key={idx}
                className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded font-medium"
              >
                {tag}
              </span>
            ))}
          </div>
        </div>
      </div>
    </Link>
  );
}